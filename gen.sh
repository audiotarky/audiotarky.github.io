source /home/metson/code/audiotarky.github.io/venv/bin/activate
python makepost.py artist Anechoics '$ilp.uphold.com/xZEy2Dg9nBMk'
python makepost.py album Anechoics 'Live at the Greenbank'
python makepost.py track Anechoics 'Connections' 'Connections 1 (propagating clocks)'
python makepost.py track Anechoics 'Connections' 'Connections 2 (lines of transmission)'
python makepost.py track Anechoics 'Connections' 'Connections 3 (disinformation)'
python makepost.py track Anechoics 'Connections' 'Connections 4 (harmony)'
python makepost.py artist 'Simon Metson' '$ilp.uphold.com/Jqz4BEwQQKmp'
python makepost.py track 'Simon Metson' 'Bristronica EP' Better
python makepost.py track 'Simon Metson' 'Bristronica EP' 5Pianos
python makepost.py track 'Simon Metson' 'Bristronica EP' 'Absolutely Shocking & Lacking in Taste'
python makepost.py track 'Stapleton Archery Club' Tribes 'Chimp Down'